...
cl.exe /nologo /LD /O2 /MT /W4 /WX /EHsc- /GR- ^
    /Fo:"%OUTPUT_DIR%\multiplayer.obj" ^
    "%SCRIPT_DIR%multiplayer.cpp" ^
    /link /Brepro ^
    /OUT:"%OUTPUT_DIR%\multiplayer.asi" ^
    /PDB:"%OUTPUT_DIR%\multiplayer.pdb"
if errorlevel 1 exit /b %errorlevel%

del /q "%OUTPUT_DIR%\multiplayer.obj" 2>nul
del /q "%OUTPUT_DIR%\multiplayer.pdb" 2>nul

echo Built "%OUTPUT_DIR%\multiplayer.asi"
