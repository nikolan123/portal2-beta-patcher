"""
install the small set of additional prerelease runtime assets.
"""
...
def archive_path() -> Path:
    return resource_path(Resource("build_852_0/extra_assets", ARCHIVE_NAME))
    ...
    def apply(self, context: PatchContext, progress: ProgressCallback) -> None:
        ...
        with ZipFile(bundle) as archive:
            ...
            for index, (relative, expected_hash) in enumerate(ASSET_HASHES.items(), start=1):
                ...
                progress(ProgressEvent("852_0.extra_assets", index, total, f"Installing {relative}"))
        ...
        progress(ProgressEvent("852_0.extra_assets", total, total, "Registering the achievement particle"))
...
DEFINITION = PatchDefinition(PrereleaseAssetsPatch(), resources=(Resource('build_852_0/extra_assets', 'assets.zip', native=False),))
