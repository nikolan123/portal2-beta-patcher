...
def load_asset_allowlist() -> frozenset[str]:
    path = resource_path(Resource("build_852_0/hl2_assets", "assets.txt"))
...
def copy_selected_loose_assets(source, destination, selected_paths, cancel_event, progress):
    ...
    for index, (relative, path) in enumerate(available, start=1):
        ...
        if index == 1 or index == total or index % 25 == 0:
            progress(ProgressEvent("852_0.hl2_assets", index, max(total, 1), f"HL2 loose file: {relative}"))
...
DEFINITION = PatchDefinition(Hl2AssetsPatch(), requirements=frozenset({"hl2"}), progress_range=(0.58, 0.88), resources=(Resource('build_852_0/hl2_assets', 'assets.txt', native=False),))
